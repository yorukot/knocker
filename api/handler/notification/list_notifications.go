...
// @Success 200 {object} response.SuccessResponse "Notifications retrieved successfully"
...
// @Router /teams/{teamID}/notifications [get]
func (h *NotificationHandler) ListNotifications(c echo.Context) error {
	...
	notifications, err := repository.ListNotificationsByTeamID(c.Request().Context(), tx, teamID)
	if err != nil {
		zap.L().Error("Failed to list notifications", zap.Error(err))
	...
	return c.JSON(http.StatusOK, response.Success("Notifications retrieved successfully", notifications))
}

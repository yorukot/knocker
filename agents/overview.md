...
5. Incident evaluation runs immediately after each ping inside `processIncident` (`worker/handler/monitor_ping.go`). See `agents/monitoring.md` for thresholds, event rules, and when notifications fire.
